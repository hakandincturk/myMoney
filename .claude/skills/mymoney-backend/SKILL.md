---
name: mymoney-dev
description: myMoney backend development skill - enforces N-layered architecture, coding conventions, DTO patterns, specification filters, event system, and all project standards for the Java 21 Spring Boot personal finance management application
when_to_use: When writing new code, adding endpoints, creating services, modifying entities, writing tests, or making any code changes in the myMoney-backend project
...
paths: src/**/*.java, pom.xml, src/main/resources/**
effort: high
---
...
> **Code patterns:** Loaded on demand from `patterns/` — each step below references the relevant pattern file
...
### Step 1: Define the DTOs → See [patterns/dto-mapper.md](patterns/dto-mapper.md)
...
### Step 2: Add Sort Column Enum (if paginated) → See [patterns/data.md](patterns/data.md)
- Create in `core/enums/sort/`, implement `BaseSortColumn` interface
- Map DTO column names to actual JPA entity field paths

### Step 3: Add Specification (if filtered) → See [patterns/data.md](patterns/data.md)
...
### Step 4: Add Repository Method (if new query needed) → See [patterns/data.md](patterns/data.md)
...
### Step 5: Add/Update Mapper → See [patterns/dto-mapper.md](patterns/dto-mapper.md)
- Add conversion method in `mappers/`, use `@Mapper(componentModel = "spring")`
- Use `@Mapping` for non-trivial field paths

### Step 6: Add Business Rules (if validation needed) → See [patterns/service.md](patterns/service.md)
...
### Step 7: Add Factory Methods (if complex creation needed) → See [patterns/service.md](patterns/service.md)
- Add to `factories/`, encapsulate creation logic, return entities (not DTOs)

### Step 8: Implement Service → See [patterns/service.md](patterns/service.md)
...
### Step 9: Implement Controller → See [patterns/controller.md](patterns/controller.md)
...
### Step 10: Add Flyway Migration (if schema change) → See [patterns/infra.md](patterns/infra.md)
...
### Step 11: Add Event + Listener (if side effects needed) → See [patterns/event.md](patterns/event.md)
...
## Workflow: Writing Tests → See [patterns/infra.md](patterns/infra.md)
